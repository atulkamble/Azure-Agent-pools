...
if [[ ${1:-} == "-h" || ${1:-} == "--help" || $# -lt 2 ]]; then
  ...
  echo "Example: $0 https://dev.azure.com/contoso BuildPool mac-builder-01"
  exit 1
fi
...
AGENT_PACKAGE="vsts-agent-osx-x64-${AGENT_VERSION}.tar.gz"
DOWNLOAD_URL="https://vstsagentpackage.azureedge.net/agent/${AGENT_VERSION}/${AGENT_PACKAGE}"
AGENT_HOME="${AGENT_HOME:-$HOME/azdo/macos-agent}"
...
if ! command -v curl >/dev/null 2>&1; then
  echo "curl is required but not found. Install Command Line Tools (xcode-select --install) or Homebrew curl." >&2
  exit 1
fi
...
pushd "vsts-agent-osx-x64-${AGENT_VERSION}" >/dev/null
